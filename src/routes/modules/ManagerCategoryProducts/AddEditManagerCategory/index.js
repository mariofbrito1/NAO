...
import { addCategoryProduct, updateCategoryProduct } from '../../../../redux/actions/Benefits';
...
const CategoryAdd = () => {
  ...
  useEffect(() => {
    const isEdit = localStorage.getItem('categoryproductedit');
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      const cat = localStorage.getItem('categoryproduct');
  ...
  const onCreateCategory = () => {
    dispatch(
      addCategoryProduct(category, data => {
        history.push('/products/management_category');
      }),
    );
  };
  ...
  const onUpdateCategory = () => {
    dispatch(
      updateCategoryProduct(category, data => {
        history.push('/products/management_category');
      }),
    );
  };
        ...
        <Box className={classes.pageFull}>
          ...
          <GridContainer>
            <Grid item xs={12} lg={12} className={classes.BenefitSidebar}>
              <Box mb={12}>
                <CmtCard>
                  <CmtCardHeader title={isEdit ? 'Edición de Categoría de Productos' : 'Nueva Categoría de Productos'} />
                  <CmtCardContent>
                      ...
                      {false && (
                        <Grid item xs={12} lg={12} styles={{ margin: '30px' }}>
                          <TextField
                            ...
                            minRows={3}
                            onChange={ev => setCategory({ ...category, descriptions: ev.target.value })}

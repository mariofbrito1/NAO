...
import { deleteCategory } from '../../../../redux/actions/Benefits';
  ...
  const handleConfirmDelete = () => {
    ...
    if (data) {
      ...
      for (let i = 0; i < selected.length; i++) {
        const dato = data.filter(d => d.id === selected[i]);
        dispatch(
          deleteCategory(dato[0], () => {
            dat = dat.filter(d => d.id !== selected[i]);
            setData(dat);
          }),
        );
      }
  ...
  return (
    <React.Fragment>
      <Toolbar
        ...
        {numSelected > 0 ? (
          ...
          <Typography className={classes.title} variant="h4" id="tableTitle" component="div">
            Listado de Categorías de Beneficios{'  '}
            <Button color="primary" onClick={() => onNewCategory()}>
              Nueva Categoría
            </Button>
          </Typography>
        )}
      ...
      <ConfirmDialog
        ...
        content={'Está seguro de que desea eliminar las categorías seleccionadas?'}
